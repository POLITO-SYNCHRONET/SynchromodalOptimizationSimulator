...
//                System.out.println(dao.getAllCalendars());                
//                System.out.println(dao.getCalendarDatesForServiceId(AgencyAndId.convertFromString("gtfs_ALWAYS")));
                for(ServiceCalendar sc: dao.getAllCalendars()){
//                	LOG.info("Check "+sc.getId());
                	graph.calendarById.put(sc.getServiceId(), sc);
//                	System.out.println(dao.getCalendarDateForId(sc.getId()));
                }
//                System.out.println(dao.getAllCalendarDates());
                for(ServiceCalendarDate scd: dao.getAllCalendarDates()){
//                	LOG.info("Check "+scd.getId());
//                	LOG.info("Check "+scd.getServiceId());
                	graph.calendarDateById.put(scd.getServiceId(), scd);
                }
        ...
        LOG.info("calendar service data : "+data.getServiceIds());
    ...
    private void loadBundle(GtfsBundle gtfsBundle, Graph graph, GtfsMutableRelationalDao dao)
        ...
        //Riccardo: adding driver rest time
//        for(StopTime stopTime : store.getAllEntitiesForType(StopTime.class)) {
//        	if(GtfsLibrary.getTraverseMode(stopTime.getTrip().getRoute()) == TraverseMode.BUS && stopTime.getDepartureTime() != 0){
//        		DriverRestModule.addRestTime(stopTime);
//        	}
//        }
        //Riccardo: END
        ...
        for (Route route : store.getAllEntitiesForType(Route.class)) {
        	//LOG.info("start reading entities route {}", route.getLongName()+" Id is "+reader.getDefaultAgencyId()+reader.getAgencies().toString());
        ...
        for (ServiceCalendar serviceCalendar : store.getAllEntitiesForType(ServiceCalendar.class)) {
        	//LOG.info("start reading entities serviceCalendar {}", serviceCalendar.getServiceId());
