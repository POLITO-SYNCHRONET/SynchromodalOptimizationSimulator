...
public class EuclideanRemainingWeightHeuristic implements RemainingWeightHeuristic {
    ...
    @Override
    public void initialize(RoutingRequest options, long abortTime) { //PHUONG: this function will be called @Astar.startSearch
    	LOG.info("INITIALIZE");
        ...
        walkReluctance = req.walkReluctance; //walkReluctance = 2 @ RoutingRequest:  A multiplier for how bad walking is, compared to being in transit for equal lengths of time.
    }
    ...
    public double estimateRemainingWeight (State s) {
    	LOG.info("ESTIMATE REMAINING WEIGHT");
        Vertex sv = s.getVertex();
        double euclideanDistance = SphericalDistanceLibrary.fastDistance(sv.getLat(), sv.getLon(), lat, lon);
        if (transit) {
            if (euclideanDistance < requiredWalkDistance) { //PHUONG: requiredWalkDistance is the walking distance that the user has to walk from the last station of the public transportation in the user's trip to the destination that the user wants to reach]
                return walkReluctance * euclideanDistance / maxStreetSpeed; //walkReluctance = 2 @ RoutingRequest:  A multiplier for how bad walking is, compared to being in transit for equal lengths of time.
            }
    ...
    /**
     * Figure out the minimum amount of walking to reach the destination from transit. [PHUONG: transit here is the last station when taking the public transportation, then we need to walk to the destination --> determineRequiredWalkDistance is that value: walking from the last station to the destination]
    ...
    private double determineRequiredWalkDistance(final RoutingRequest req) {
    	LOG.info("DETERMINE REQUIRED WALK DISTANCE");
    ...
    private class ClosestStopTraverseVisitor implements TraverseVisitor {
        ...
        @Override public void visitEdge(Edge edge, State state) {    	LOG.info("VISIT EDGE");
 }
        @Override public void visitEnqueue(State state) {    	LOG.info("VISIT ENQUEUE");
 }
    ...
    @Override
    public void reset() {    	LOG.info("RESET");
}

    @Override
    public void doSomeWork() {    	LOG.info("DO SOME WORK");
}
