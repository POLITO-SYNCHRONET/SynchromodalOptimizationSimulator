...
public class TripTimes implements Serializable, Comparable<TripTimes>, Cloneable {
    ...
    public final Trip trip; //PHUONG: from org.onebusaway.gtfs.model.Trip [trips.txt file in GTFS]
    ...
    //double[] shape_dist_traveled; //PHUONG: to get this field from stop_times.txt of GTFS
    private final int[] shape_dist_traveled;//EDIT - Riccardo (08-05-2017): read distance from GTFS
    ...
    public TripTimes(final Trip trip, final List<StopTime> stopTimes, final Deduplicator deduplicator) {
        ...
        final int[] distance = new int[nStops];//EDIT - Riccardo (08-05-2017): read distance from GTFS
        ...
        timeShift = stopTimes.get(0).getArrivalTime(); //PHUONG: = the arrival time at the departure point
        int s = 0;
        for (final StopTime st : stopTimes) {
            ...
            distance[s] = (int) st.getShapeDistTraveled();//EDIT - Riccardo (08-05-2017): read distance from GTFS
            s++;
        }
        this.shape_dist_traveled = deduplicator.deduplicateIntArray(distance);//EDIT - Riccardo (08-05-2017): read distance from GTFS
    ...
    public TripTimes(final TripTimes object) {
        ...
        this.shape_dist_traveled = object.shape_dist_traveled;//EDIT - Riccardo (08-05-2017): read distance from GTFS
    }
    
    //EDIT - Riccardo (08-05-2017): read distance from GTFS
    public int getDistance(int stopIndex){
    	return shape_dist_traveled[stopIndex];
    }
    //EDIT - Riccardo (08-05-2017)
    ...
    public boolean tripAcceptable(final State state0, final int stopIndex) {
        ...
        if (banned != null && banned.contains(stopIndex)) { //PHUONG: if trip is banned --> != null; && stopIndex is a stop of banned
            return false;
        }
    ...
    public boolean tripAcceptablePHUONG(final State state0, final int stopIndex) {
        final RoutingRequest options = state0.getOptions();
        final BannedStopSet banned = options.bannedTrips.get(trip.getId());
        if (banned != null && banned.contains(stopIndex)) { //PHUONG: if trip is banned --> != null; && stopIndex is a stop of banned
            LOG.info("Ohh belong to Banned trip {}",trip.getId().getId());
        	return false;
        }
        if (options.wheelchairAccessible && trip.getWheelchairAccessible() != 1) {
            return false;
