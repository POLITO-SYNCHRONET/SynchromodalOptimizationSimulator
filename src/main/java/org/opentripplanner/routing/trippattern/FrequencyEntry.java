...
public class FrequencyEntry implements Serializable {
    ...
    public int nextDepartureTime (int stop, int time) {
    	int timeInDay = time%86400; // MASSIMO & GIUSEPPE
        if ( (timeInDay) > endTime) return -1;
        ...
        if (exactTimes) {
            ...
            int dep = timeInDay + headway;
            int offset = time - timeInDay;
            
            // TODO it might work better to step forward until in range
            // this would work better for time window edges.
            if (dep < beg) return beg+ offset; // not quite right
            if (dep < end) return dep + offset;
        }
        return -1;
    }
    ...
    public int prevArrivalTime (int stop, int t) {
        ...
        if (exactTimes) {
        	...
        	int timeInDay = t % 86400; // MASSIMO & GIUSEPPE
            int offset = t - timeInDay;
            int dep = timeInDay - headway;
            if (dep > end) return end+offset; // not quite right
            if (dep > beg) return dep+offset;
        }
        return -1;
    }
