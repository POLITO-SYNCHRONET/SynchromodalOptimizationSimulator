...
public class TransferEdge extends Edge {
    ...
    /**
     * Creates a new Transfer edge. : PHUONG: Create transfer edges between stops which are listed in transfers.txt of GTFS feed
    ...
    public State traverse(State s0) {
        ...
        double cost_timeP = options.cost_per_hour * time/3600.0;
        double increaseP = options.time_w  * cost_timeP;
        s1.incrementWeight(increaseP);
        s1.incrementcostTime(cost_timeP);
        //s1.incrementWeight(time); //PHUONG: the minimum time in seconds it takes to complete this transfer: A transfer directly between two stops without using the street network [defined in transfer.txt in GTFS]
