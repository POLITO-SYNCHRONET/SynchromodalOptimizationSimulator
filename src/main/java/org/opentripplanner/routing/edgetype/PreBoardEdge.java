...
public class PreBoardEdge extends FreeEdge implements StationEdge {
    ...
    public State traverse(State s0) {
        ...
        if (options.arriveBy) {
            ...
            slack = 0;//Riccardo (06-06-2017): we don't need transfer time here
           ...
           // s1.incrementWeight(wait_cost); //PHUONG added on Nov14
            /*PHUONG: s1.incrementWeight(wait_cost + transfer_penalty); */
            ...
            if (!options.forcedStops.isEmpty()) {
            	if (s0.existforcedStop)  //PHUONG: there already exists the forcedStops on the path from the initial stop up to this state s0
            	{
            		 forced = true;
            		 double cost_timeP =  options.cost_per_hour * wait_cost/3600.0;
            	...
            	{
		            if (options.forcedStops.matches(((TransitStop) fromv).getStop())) { //PHUONG: there does not exist the forcedStops on the path from the initial stop up to this state0 -->need to check whether fromv is the forcedStop
		            	 forced = true;
	            		 double cost_timeP =  options.cost_per_hour * wait_cost/3600.0;
	            		 double increaseP = options.time_w  * cost_timeP;
	            	     s1.incrementWeight(increaseP);
		            ...
		            else // there does not exist the forcedStops on the path from the initial stop up to this state s0 AND fromv is NOT the forcedStop --> penalty 10 times 
		            {
	            		 double cost_timeP =  options.cost_per_hour * wait_cost/3600.0;
	            		 double increaseP = options.time_w  * cost_timeP;
	            	     //s1.incrementWeight(10*increaseP);
	            		 s1.incrementWeight(increaseP);
	            	     s1.incrementcostTime(cost_timeP);
		            }
            	}
