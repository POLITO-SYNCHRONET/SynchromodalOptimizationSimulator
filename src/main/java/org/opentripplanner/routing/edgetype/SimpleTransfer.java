...
public class SimpleTransfer extends Edge {
    ...
    public State traverse(State s0) {
        ...
        int time = (int) Math.ceil(distance / walkspeed) + 2 * StreetTransitLink.STL_TRAVERSE_COST; //PHUONG: STL_TRAVERSE_COST = 1 in StreetTransitLink.java
        time = 0; //Riccardo (06-06-2017): we don't need transfer time here
        se.incrementTimeInSeconds(time);
        double increaseP = rr.time_w * time * rr.cost_per_hour;
	    se.incrementWeight(increaseP);
        //se.incrementWeight(time * rr.walkReluctance); //PHUONG:  A multiplier for how bad walking is, compared to being in transit for equal lengths of time. Defaults to 2.
