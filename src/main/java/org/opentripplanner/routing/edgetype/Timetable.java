...
public class Timetable implements Serializable {
    ...
    public TripTimes getNextTrip(State s0, ServiceDay serviceDay, int stopIndex, boolean boarding) {
        ...
        for (TripTimes tt : tripTimes) {
            ...
            if ( ! tt.tripAcceptable(s0, stopIndex)) continue; //PHUONG: tripAcceptable = false if Triptimes tt is banned and stopIndex is a stop of tt [see TripTimes.java] 
        ...
        for (FrequencyEntry freq : frequencyEntries) {
            ...
            if (boarding) {
            	...
            	//Riccardo: remove this always allowed to take the next trip (still need to understand why)
//                if (depTime < 0) continue; 
    ...
    public TripTimes getNextTripPHUONG(State s0, ServiceDay serviceDay, int stopIndex, boolean boarding, TripPattern tripPattern) {
        /* Search at the state's time, but relative to midnight on the given service day. */
        int time = serviceDay.secondsSinceMidnight(s0.getTimeSeconds());
        //long timeP = s0.getTimeSeconds();
        //LOG.info("timeState {}, timeServiceDay {}",timeP,time);
        // NOTE the time is sometimes negative here. That is fine, we search for the first trip of the day.
        // Adjust for possible boarding time TODO: This should be included in the trip and based on GTFS
        if (boarding) {
            time += s0.getOptions().getBoardTime(this.pattern.mode);
        ...
        TripTimes bestTrip = null;
        Stop currentStop = pattern.getStop(stopIndex);
        LOG.info("Boarding = {}, TimeState = {} ~ TimeServiceDay {}, stop = {}",boarding,s0.getTimeSeconds(),time,currentStop.getName());
        ...
        //int ccP = 0;
        for (TripTimes tt : tripTimes) {
        	//ccP++; LOG.info("Inside TripTimes lan thu {}",ccP);
            if (tt.isCanceled())
            	{
            	 LOG.info("tt cancel");
            	continue;
            ...
            if ( ! tt.tripAcceptable(s0, stopIndex)) 
            	{
            	 LOG.info("Trip not accept");
            	continue; //PHUONG: tripAcceptable = false if Triptimes tt is banned and stopIndex is a stop of tt [see TripTimes.java] 
            	}
            int adjustedTime = adjustTimeForTransfer(s0, currentStop, tt.trip, boarding, serviceDay, time);
            LOG.info("adjustTime = {} for trip {} of route {}",adjustedTime,tt.trip.getId().getId(),tt.trip.getRoute().getId().getId());
            if (adjustedTime == -1) continue;
            if (boarding) {
                int depTime = tt.getDepartureTime(stopIndex);
                LOG.info("Next depart = {} of routeID {} (stopIndex = {}); adjustTime = {}",depTime,tt.trip.getRoute().getId().getId(),stopIndex,adjustedTime);
                if (depTime < 0) continue; // negative values were previously used for canceled trips/passed stops/skipped stops, but
                                           // now its not sure if this check should be still in place because there is a boolean field
                                           // for canceled trips
                if (depTime >= adjustedTime && depTime < bestTime) {
                ...
                int arvTime = tt.getArrivalTime(stopIndex);
                LOG.info("Next arrival = {} of routeID {} (stopIndex = {}); adjustTime = {}",arvTime,tt.trip.getRoute().getId().getId(),stopIndex,adjustedTime);
                 if (arvTime < 0) continue;
                if (arvTime <= adjustedTime && arvTime > bestTime) {
                    bestTrip = tt;
                    bestTime = arvTime;
        ...
        for (FrequencyEntry freq : frequencyEntries) {
        	countP++;
        	LOG.info("Frequency lan thu {}",countP);
            TripTimes tt = freq.tripTimes;
            if (tt.isCanceled())
            	{
            		LOG.info("tt canceled");
            	continue;
            	}
            if ( ! serviceDay.serviceRunning(tt.serviceCode)) 
            	{
            	    LOG.info("serviceRutinning");
            	continue; // TODO merge into call on next line
            	}
            if ( ! tt.tripAcceptablePHUONG(s0, stopIndex)) 
            	{
            	   LOG.info("Trip {} of route {} not accept",tt.trip.getId().getId(),tt.trip.getRoute().getId().getId());
            	   continue;
            	}
            int adjustedTime = adjustTimeForTransfer(s0, currentStop, tt.trip, boarding, serviceDay, time);
            if (adjustedTime == -1)
            	{
            	   LOG.info("adjustedTime");
            	    continue;
            	}
            LOG.debug("  running freq {}", freq);
            ...
            if (boarding) {
                int depTime = freq.nextDepartureTime(stopIndex, adjustedTime); // min transfer time included in search
                LOG.info("FREQUENCY {}: Next depart = {} of routeID {} (stopIndex = {}); adjustTime = {}",countP,depTime,tt.trip.getRoute().getId().getId(),stopIndex,adjustedTime);
                if (depTime < 0) continue; 
                if (depTime >= adjustedTime && depTime < bestTime) {
                    bestFreq = freq;
                    bestTime = depTime;
                ...
                int arvTime = freq.prevArrivalTime(stopIndex, adjustedTime); // min transfer time included in search
                if (arvTime < 0) continue;
                LOG.info("FREQUENCY {}: Next arrival = {} of routeID {} (stopIndex = {}); adjustTime = {}",countP,arvTime,tt.trip.getRoute().getId().getId(),stopIndex,adjustedTime);
                if (arvTime <= adjustedTime && arvTime > bestTime) {
                    bestFreq = freq;
                    bestTime = arvTime;
                }
...
//        	System.out.println(tt.getArrivalTime(0) + " : " + tt.getDepartureTime(0));
//        	System.out.println(tt.getArrivalTime(1) + " : " + tt.getDepartureTime(1));
            for (int h = 0; h < nHops; ++h) {
                int dt = tt.getDwellTime(h);
                if (minDwellTimes[h] > dt) {
                    minDwellTimes[h] = dt;
                }
                int rt = tt.getRunningTime(h); //return getArrivalTime(h + 1) - getDepartureTime(h); from TripTimes
