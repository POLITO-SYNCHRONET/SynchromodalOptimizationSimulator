...
public class PreAlightEdge extends FreeEdge implements StationEdge {
    ...
    public State traverse(State s0) {
        ...
        //PHUONG: HERE, we will check preferredStop is matched or not; if not mached --> increment larger amount of Weight 
...
//            long alight_before = t0 - slack;
          //Riccardo (06-06-2017): we don't need transfer time here
            
          long alight_before = t0;
            ...
            s1.incrementWeight(wait_cost); //PHUONG
           ...
           /* boolean forced = false;
            if (!options.forcedStops.isEmpty()) 
            {
            	...
            	{
            		if (options.forcedStops.matches(((TransitStop) tov).getStop())) {
                    	forced = true;
                    	s1.incrementWeight(wait_cost + transfer_penalty);
                    }
            		else s1.incrementWeight(10*(wait_cost + transfer_penalty));
            	}
            }
            else s1.incrementWeight(wait_cost + transfer_penalty);*/
            ...
            TransitStop toVertex = (TransitStop) getToVertex();
            s1.alightTransit();
            s1.incrementTimeInSeconds(options.alightSlack);  //PHUONG: currently set = 0 at RoutingRequest.java
            /*double increaseP = options.time_w * options.alightSlack * options.cost_per_hour;
            s1.incrementWeight(increaseP); do we need it ? PHUONG added Dec5*/ 
            s1.setBackMode(getMode());
            return s1.makeStatePHUONG(s0.existforcedStop);
        }
    }
