...
public class RoutingRequest implements Cloneable, Serializable {
    ...
    public ArrayList<GenericLocation> passByCities = new ArrayList<>(); //EDIT - Riccardo (19/04/2017): pass by cities
     ...
     */
    public int maxPreTransitTime = 30 * 60; //PHUONG: used for StreetEdge.java in doTraverse()
    ...
    public long dateTime = new Date().getTime() / 1000; //PHUONG: the number of seconds since January 1, 1970, 00:00:00 GMT represented by this Date object

    /** Whether the trip should depart at dateTime (false, the default), or arrive at dateTime. */
    public boolean arriveBy = false;

    /** Whether the trip must be wheelchair accessible. */
    public boolean wheelchairAccessible = false; //PHUONG: used for PathwayEdge.java in traverse()

    /** The maximum number of itineraries to return. */
    public int numItineraries = 15;
    ...
    public boolean driverRestTime = false;
    
    //Giovanni capacity variable request
    public int capacityRequest = 0;
    //END Giovanni
     ...
     */
    public int transferPenalty = 0; //PHUONG: used for PreAlightEdge.java in traverse(); ?? difference between transferPenalty and nonpreferredTransferPenalty
    
    /** Penalty for using a non-preferred transfer */
    public int nonpreferredTransferPenalty = 180; //??PHUONG: to reduce the number of transfer??? NO (already have variable transferPenalty)
                                                  //is used in TransitBoardAlight.java for traverse()
    ...
    public double walkReluctance = 2.0; //PHUONG: used at SimpleTransfer.java for traverse() & at StreetEdge.java for doTraverse()

    /** Used instead of walk reluctance for stairs */
    public double stairsReluctance = 2.0; //PHUONG: used for StreetEdge.java in doTraverse()
    
    /** Multiplicative factor on expected turning time. */
    public double turnReluctance = 1.0; //PHUONG: used for StreetEdge.java in doTraverse()
    ...
    public int elevatorBoardTime = 1; //90;

    /** What is the cost of boarding an elevator? */
    public int elevatorBoardCost = 1;//90;

    /** How long does it take to advance one floor on an elevator? */
    public int elevatorHopTime = 1; //20; //PHUONG: for ElevatorHopEdge.java in traverse()

    /** What is the cost of travelling one floor on an elevator? */
    public int elevatorHopCost = 1;// 20;
    ...
    /** Time to rent a bike */
    public int bikeRentalPickupTime = 1;// 60; //PHUONG: used at RentABikeAbstractEdge.java

    /**
     * Cost of renting a bike. The cost is a bit more than actual time to model the associated cost and trouble.
     */
    public int bikeRentalPickupCost = 1;//120;

    /** Time to drop-off a rented bike */
    public int bikeRentalDropoffTime = 1;//30; //PHUONG: used at RentABikeAbstractEdge.java

    /** Cost of dropping-off a rented bike */
    public int bikeRentalDropoffCost = 1;//30;

    /** Time to park a bike */
    public int bikeParkTime = 1;//60; //PHUONG: for BikeParkEdge.java in traverse()

    /** Cost of parking a bike. */
    public int bikeParkCost = 1;//120; //PHUONG: for BikeParkEdge.java in traverse()

    /**
     * Time to park a car in a park and ride, w/o taking into account driving and walking cost
     * (time to park, switch off, pick your stuff, lock the car, etc...)
     */
    public int carDropoffTime = 1;//120; //PHUONG: for ParkAndRideEdge.java in traverse()
     ...
     */
    public double waitReluctance = 1.0; //used for TransitBoardAlight.java in traverse()

    /** How much less bad is waiting at the beginning of the trip (replaces waitReluctance on the first boarding) */
    public double waitAtBeginningFactor = 0.4; //PHUONG: e.g: TIME_TO_WAIT =  wait_at_the_beginning_of_the_trip_in_seconds * waitAtBeginningFactor;
                                               //used for TransitBoardAlight.java in traverse()

    /** This prevents unnecessary transfers by adding a cost for boarding a vehicle. */
    public int walkBoardCost = 1;//60 * 10; //PHUONG: used for TransitBoardAlight in traverse()

    /** Separate cost for boarding a vehicle with a bicycle, which is more difficult than on foot. */
    public int bikeBoardCost = 1;//60 * 10; //PHUONG: used for TransitBoardAlight in traverse()
    ...
    // ------------------------------------- New SynchroNET values: --------------------------------------------------------
        
    //PHUONG: forcedStops insert it here
    public StopMatcher forcedStops = StopMatcher.emptyMatcher(); 
    public int sortResultType = 0; //sort Results presented on the web use-interface: 0: weight ascending; 1: C02 ascending; 2:duration ascending,  3: distance ascending; 4:departure ascending; 5: arrival ascending
    /** The epoch date/time that the trip should depart (or arrive, for requests where arriveBy is true) */
    public long dateTimeArrive = new Date().getTime() / 1000 + 86400;
    public boolean timeArriveVisible = false;
    public boolean slowSteaming = false;
    public boolean KPIimportance = false;

    
    public int choiceOPT = 1;
    
    public double cost_distance_per_km_train = 0; //cost in euro per km for train
    public double cost_distance_per_km_truck = 0; //cost in euro per km for truck
    public double cost_distance_per_km_ship = 0; //cost in euro per km for ship
    public double cost_per_hour = 0; //cost in euro per 1hour of driving/waiting
    public double co2_cost_per_kg = 0; //cost in euro per 1 kg CO2
    public double co2_per_km_train = 0; //kg CO2 per km train 
    public double co2_per_km_truck = 0; //kg CO2 per km truck
    public double co2_per_km_feeder_ship = 0; //kg CO2 per km ship (medium velocity: 15knots)
    public double co2_per_km_feeder_ship_Slow = 0; //kg CO2 per km ship (slow velocity: 10knots)
    public double co2_per_km_feeder_ship_Medium = 0; //kg CO2 per km ship (medium velocity: 15knots)
    public double co2_per_km_feeder_ship_Fast = 0; //kg CO2 per km ship (fast velocity: 20knots)
    public double co2_per_km_big_ship = 0;  //kg CO2 per km ship (medium velocity: 15knots)
    public double co2_per_km_big_ship_Slow = 0; //kg CO2 per km ship (slow velocity: 10knots)
    public double co2_per_km_big_ship_Medium = 0; //kg CO2 per km ship (medium velocity: 15knots)
    public double co2_per_km_big_ship_Fast = 0; //kg CO2 per km ship (fast velocity: 20knots)
    
    //Riccardo boarding (min), alighting (min) and capacity (container) values
    public int boarding_time_truck = 0;
    public int boarding_time_rail = 0;
    public int boarding_time_ship_lolo = 0;
    public int boarding_time_ship_roro = 0;
    public int alighting_time_truck = 0;
    public int alighting_time_rail = 0;
    public int alighting_time_ship_lolo = 0;
    public int alighting_time_ship_roro = 0;
    public int capacity_rail = 0;
    public int capacity_feeder_ship = 0;
    public int capacity_oceanic_ship = 0;
    
    //Riccardo: changed ship co2 based on CRAIN simulation (prev: 0.01/0.02/0.04)
    //Truck changed from 0.137 to be more precise
    
    public double distance_w = 1.0;  //weight on the distance in generalized cost
    public double co2_w = 1.0;  //weight on the CO2 in generalized cost
    public double time_w = 1.0;  //weight on the travel time in generalized cost
    
    //Generalized cost = distance_w * cost_distance_per_km_train/truck/ship * distance + co2_w * co2_cost_per_kg * co2_per_km_train/truck/ship * distance + time_w * cost_pre_hour * traveltime
    // Yuanyuan(22-11-2017): KRIs
    public double KRI_time_w = 1.0;
    public double KRI_flexibility_w = 1.0;
    public double KRI_safety_w = 1.0;
    public double KRI_cost_w = 1.0;
    
    //if true try to avoid legs that don't fit the requested capacity
    public boolean enoughCapacity = false;
    public boolean fromTruckToTruck = false;
    ...
    public int alightSlack = 0; //PHUONG: used at preAlightEdge.java for traverse()
    ...
    public double softWalkPenalty = 60.0; // a jump in cost when stepping over the walking limit //PHUONG: used for StreetEdge.java in doTraverse()
    public double softWalkOverageRate = 5.0; // a jump in cost for every meter over the walking limit //PHUONG: used for StreetEdge.java in doTraverse()

    public double preTransitPenalty = 300.0; // a jump in cost when stepping over the pre-transit time limit //PHUONG: used for StreetEdge.java in doTraverse()
    public double preTransitOverageRate = 10.0; // a jump in cost for every second over the pre-transit time limit //PHUONG: used for StreetEdge.java in doTraverse()
    ...
    */
    ...
    public void setArriveTimeVisible(boolean arriveTimeVisible) {
    	this.timeArriveVisible = arriveTimeVisible;
    }
    ...
    public void setForcedStops(String s) {
    	
        if (s != null && !s.equals("")) {
            forcedStops = StopMatcher.parse(s);
        }
        else {
            forcedStops = StopMatcher.emptyMatcher();
        }
    } 
    ...
    //EDIT - Riccardo (19/04/2017): Save pass by stops
    public void setPassByStops(String passBy){
    	for(String city : passBy.split(";")){
        	this.passByCities.add(GenericLocation.fromOldStyleString(city));
    	}
    }
    //EDIT - Riccardo (19/04/2017): END
    
    public void setchoiceOPT(int choiceOPT) {
        this.choiceOPT = choiceOPT;
    }
    ...
    public Date getDateTimeArrive() {
        return new Date(dateTimeArrive * 1000);
    }
    ...
    public void setDateTimeArrive(Date dateTimeArrive) {
        this.dateTimeArrive = dateTimeArrive.getTime() / 1000;
    }
    ...
    public void setDateTimeArrive(String date, String time, TimeZone tz) {
        Date dateObject = DateUtils.toDate(date, time, tz);
        setDateTimeArrive(dateObject);
    }
    ...
    public RoutingRequest clone() {
        try {
            ...
            clone.forcedStops = forcedStops.clone();
    ...
    public boolean equals(Object o) {
        ...
        return endpointsMatch
                ...
                && dateTimeArrive == other.dateTimeArrive
                ...
                && timeArriveVisible == other.timeArriveVisible
                && slowSteaming == other.slowSteaming
                && KPIimportance == other.KPIimportance
                && optimize.equals(other.optimize)
                && maxWalkDistance == other.maxWalkDistance
                && maxPreTransitTime == other.maxPreTransitTime
                && sortResultType == other.sortResultType
                ...
                && co2_w  == other.co2_w              
                && time_w  == other.time_w           
                &&	co2_cost_per_kg == other.co2_cost_per_kg
                &&	cost_per_hour == other.cost_per_hour
                &&	cost_distance_per_km_truck == other.cost_distance_per_km_truck
                &&	cost_distance_per_km_train == other.cost_distance_per_km_train
                &&	cost_distance_per_km_ship == other.cost_distance_per_km_ship
                && choiceOPT == other.choiceOPT
                && KRI_time_w == other.KRI_time_w
                && KRI_cost_w == other.KRI_cost_w
                && KRI_flexibility_w == other.KRI_flexibility_w
                && KRI_safety_w == other.KRI_safety_w;
    }
    ...
    public int hashCode() {
        int hashCode = new Double(walkSpeed).hashCode() + new Double(bikeSpeed).hashCode()
                ...
                + (timeArriveVisible ? 3966786 : 0) +
                + (slowSteaming ? 3966773 : 0) +
                + (KPIimportance ? 5966773 : 0) +
                ...
                + new Double(distance_w).hashCode() * 15485851
                + new Double(co2_w).hashCode() * 15485873
                + new Double(time_w).hashCode() * 15485863
                // Yuanyuan (22-11-2017): add KRI
                + new Double(KRI_time_w).hashCode() * 1818188
                + new Double(KRI_flexibility_w).hashCode() * 1818678
                + new Double(KRI_safety_w).hashCode() * 1238188
                + new Double(KRI_cost_w).hashCode() * 1435188
                + new Double(co2_cost_per_kg).hashCode() * 17485863
                + new Double(cost_per_hour).hashCode() * 17485873
                + new Double(cost_distance_per_km_truck).hashCode() * 27485873
                + new Double(cost_distance_per_km_train).hashCode() * 17485877
                + new Double(cost_distance_per_km_ship).hashCode() * 37485873
                + walkBoardCost + bikeBoardCost + choiceOPT + bannedRoutes.hashCode()
    ...
    public int getBoardCost(TraverseMode mode) {
        ...
        return 0; //PHUONG: just used for transit GTFS [change in TransitBoardAlight.java]
    }
    ...
    public int getBoardTime(TraverseMode transitMode) {
    	Integer i = null;
    	try {
        i = this.rctx.graph.boardTimes.get(transitMode);
    	} catch (NullPointerException e) {
    		LOG.info("transitMode is "+transitMode+" i is "+i);
    		e.printStackTrace();
    	}
        return i == null ? 0 : i;
    }
     ...
     */
    public double getTransitSpeedUpperBound() { //PHUONG: only use for EuclideanRemainingheuristic
    ...
    public double getcost_distance_per_kmLowerBound() { //PHUONG: used for Astar
        double min = cost_distance_per_km_truck < cost_distance_per_km_train ?  cost_distance_per_km_truck :  cost_distance_per_km_train;
        if (min > cost_distance_per_km_ship) return cost_distance_per_km_ship; 
        return min; 
    }
    

//    public double getco2_per_kmLowerBound() { //PHUONG: used for Astar
//        double min = co2_per_km_truck < co2_per_km_train ?  co2_per_km_truck :  co2_per_km_train;
//        if (min > co2_per_km_ship) return co2_per_km_ship; 
//        return min; 
//    }
