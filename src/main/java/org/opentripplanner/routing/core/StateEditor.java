...
public class StateEditor {
    ...
    public State makeState() {
        ...
        if (defectiveTraversal) {
            ...
            LOG.info("Defective traversal flagged on edge " + child.backEdge);
        ...
        if (child.backState != null) {
            ...
            if (traversingBackward ? (child.getTimeDeltaSeconds() > 0)
                ...
                LOG.trace("Time was incremented the wrong direction during state editing. {}",
                        child.backEdge);
                LOG.info("IN STATE EDITOR: something wrong, return null");
                return null;
            }
        }
        spawned = true;
        if (child == null) LOG.info("SAO LAI NULL");
        
        return child;
    }
    
    
    public State makeStatePHUONG(boolean forced) {
        ...
        if (child.backState != null) {
            ...
            if (traversingBackward ? (child.getTimeDeltaSeconds() > 0)
                ...
                LOG.trace("Time was incremented the wrong direction during state editing. {}",
                        child.backEdge);
                LOG.info("Time was incremented the wrong direction during state editing. {}",
                        child.backEdge);
                return null;
            }
        }
        spawned = true;
        child.existforcedStop = forced;
        if (child == null) LOG.info("SAO LAI NULLPHUONG");
//System.out.println(child.toString());
    ...
    public void incrementWeight(double accumulatedWeight) {
//    	if(this.child.backEdge.getClass().equals(SimpleTransfer.class)) return;
    	if(!this.child.backEdge.getClass().equals(PatternHop.class)) return;

    	if (Double.isNaN(accumulatedWeight)) {
        ...
        if(child.weight != Double.MAX_VALUE){
        	if(accumulatedWeight == Double.MAX_VALUE) child.weight = Double.MAX_VALUE;
        	else child.weight += accumulatedWeight;
        }
//        child.weight = child.accumulatedWeight;
        
//        System.out.println(child.backEdge.getFromVertex().getLabel() + " " + child.backEdge.getToVertex().getLabel());
//        System.out.println("Total: " + child.weight + " Accumulated: " + child.accumulatedWeight + " Target: " + getWeightFromTarget());
    }
    
    public void incrementCO2(double CO2) {
        if (Double.isNaN(CO2)) {
            LOG.warn("A state's CO2 is being incremented by NaN while traversing edge "
                    + child.backEdge);
            defectiveTraversal = true;
            return;
        }
        if (CO2 < 0) {
            LOG.warn("A state's CO2 is being incremented by a negative amount while traversing edge "
                    + child.backEdge);
            defectiveTraversal = true;
            return;
        }
    ...
    public void incrementDistance(double distance) {
        if (Double.isNaN(distance)) {
            LOG.warn("A state's distance is being incremented by NaN while traversing edge "
                    + child.backEdge);
            defectiveTraversal = true;
            return;
        }
        if (distance < 0) {
            LOG.warn("A state's distance is being incremented by a negative amount while traversing edge "
                    + child.backEdge);
            defectiveTraversal = true;
            return;
        }
    ...
    public void incrementcostDistance(double costDistance) {
        child.costDistance += costDistance;
    }

    
    public void incrementcostCO2(double costCO2) {
        child.costCO2 += costCO2;
    }

    
    public void incrementcostTime(double costTime) {
        child.costTime += costTime;
    }
    
    public void incrementWAIT1(double WAIT) {
        child.WAIT1 += WAIT;
    }
    
    public void incrementWAIT2(double WAIT) {
        child.WAIT2 += WAIT;
    }
    

    public void incrementWAIT3(double WAIT) {
        child.WAIT3 += WAIT;
    }
    
    public void incrementWeight_PHUONG() {
        child.weight = 0;
    }   
    ...
    public long getElapsedTimeSeconds() {
        return child.getDurationSeconds();
    }
