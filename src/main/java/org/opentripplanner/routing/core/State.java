...
import org.opentripplanner.routing.algorithm.NegativeDistanceException;
import org.opentripplanner.common.model.NamedPlace;
import org.opentripplanner.routing.algorithm.NegativeCO2Exception;
...
public class State implements Cloneable {
    ...
    // accumulated CO2 up to this state
    public double CO2;
    public double costCO2;
    
    // accumulated distance up to this state
    public double distance;
    public double costDistance;
    public double costTime;
    
    //EDIT - Riccardo (06/06/2017): store transit time 
    ...
    //EDIT - Giovanni (20/12/2017): add capacity
    public int capacity = 0;
    //EDIT - Giovanni (20/12/2017): END
    
    //COMMENT - Riccardo (07-03-2017): Probably useless
    public double WAIT1;
    public double WAIT2;
    public double WAIT3;
    //Riccardo (07-03-2017): END
    
    //PHUONG: forcedStop: whether up to this state there is forcedStop on the way
    public boolean existforcedStop = false;
    ...
    public State(Vertex vertex, Edge backEdge, long timeSeconds, long startTime, RoutingRequest options) {
        this.weight = 0; existforcedStop = false;
        this.distance = 0; this.CO2 = 0;
        this.costDistance = 0; this.costCO2 = 0; this.costTime = 0; 
        this.WAIT1 = 0; this.WAIT2 = 0; this.WAIT3 = 0; //COMMENT - Riccardo (07-03-2017): probably useless
        this.vertex = vertex;
    ...
    public long getLastAlightedTimeSeconds() { //PHUONG: this function is called by only adjustTimeForTransfer function in the timeTable.java
    ...
    public double getCO2() { //PHUONG
        return this.CO2;
    }
    
    public double getDistance() { //PHUONG
    ...
    public double getDistanceDelta() {
        return this.distance - backState.distance;
    }    

    public void checkNegativeDistance() {
        double dw = this.distance - backState.distance;
        if (dw < 0) {
            throw new NegativeDistanceException(String.valueOf(dw) + " on edge " + backEdge);
        }
    }
    
    public double getCO2Delta() {
        return this.CO2 - backState.CO2;
    }
    
    public void checkNegativeCO2() {
        double dw = this.CO2 - backState.CO2;
        if (dw < 0) {
            throw new NegativeCO2Exception(String.valueOf(dw) + " on edge " + backEdge);
        }
    }
    ...
    public Trip getBackTrip () {
        if (backEdge instanceof TablePatternEdge) { //PHUONG: consists of PatternDwell, PatternHop, TransitBoardAlight, TrnasitBoardAlight
            return stateData.tripTimes.trip; //PHUONG: trip in the format of GTFS
        }
        else {
            return backEdge.getTrip(); //PHUONG: inside the Edge.java: it returns NULL
        }
    }
     ...
     */
    public State optimizeOrReverse (boolean optimize, boolean forward) { //PHUONG: just be called by TransitBoardAlight edge
        State orig = this;
        ...
        if (forward) {
            ...
            if (getDurationSeconds() != reversed.getDurationSeconds())
                LOG.warn("Optimization changed time: before " + this.getDurationSeconds() + " after "
                        + reversed.getDurationSeconds());
...
//                    Arrays.copyOf(this.pathParserStates, this.pathParserStates.length, newLength);
    ...
    public double getOptimizedElapsedTimeSeconds() {
        return getDurationSeconds() - stateData.initialWaitTime;
    }
    ...
    //EDIT - Riccardo (07-04-2017): real accumulated weight
   
    public void setAccumulatedWeight(double accumulatedWeight){
    	this.weight = accumulatedWeight;
    }
