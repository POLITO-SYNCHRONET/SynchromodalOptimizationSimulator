...
public class TransferTable implements Serializable {
    ...
    public void addTransferTime(Stop fromStop, Stop toStop, Route fromRoute, Route toRoute, Trip fromTrip, Trip toTrip, int transferTime) {
        ...
        if (transferTime == StopTransfer.PREFERRED_TRANSFER
                || transferTime == StopTransfer.TIMED_TRANSFER) { //PHUONG: PREFERRED_TRANSFER = -2;Special value for transferTime indicating that this transfer is preferred
        	                                                              //TIMED_TRANSFER = 0; Special value for transferTime indicating that this transfer is timed.In a timed transfer, the departing vehicle will wait for passengers from the arriving vehicle, so the minimum transfer time is effectively zero.
    ...
    public int determineTransferPenalty(int transferTime, int nonpreferredTransferPenalty) {
        ...
        if (hasPreferredTransfers()) {
            ...
            if (transferTime == StopTransfer.PREFERRED_TRANSFER
                    || transferTime == StopTransfer.TIMED_TRANSFER) { //PHUONG: PREFERRED_TRANSFER = -2;Special value for transferTime indicating that this transfer is preferred
                // Depenalize preferred transfers                               TIMED_TRANSFER = 0; Special value for transferTime indicating that this transfer is timed. In a timed transfer, the departing vehicle will wait for passengers from the arriving vehicle, so the minimum transfer time is effectively zero.
