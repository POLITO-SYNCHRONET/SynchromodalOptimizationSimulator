    ...
    public boolean add(State newState) {
        ...
        while (it.hasNext()) {
            ...
            if (dominanceFunction.betterOrEqualAndComparable(oldState, newState))
                return false;
//            if (oldState.weight <= newState.weight)
//                return false;
            if (dominanceFunction.betterOrEqualAndComparable(newState, oldState))
                it.remove();
        }
        System.out.println("remove old state");
    ...
    public boolean visit(State state) {
        boolean ret = false;
        for (State s : stateSets.get(state.getVertex())) { //PHUONG: get set of states of "vertex represented in variable state"
            if (s == state) {
                ret = true;
                break;
            }
