...
public class IndexAPI {
   ...
   //Shuai(11-04-2018) API for return routes related to the departure and arrival stop
   @GET
   @Path("/stops/{departureId}/{arrivalId}/routes")
   public Response getRoutesForDepartureAndArrival (@PathParam("departureId") String departureId, @PathParam("arrivalId") String arrivalId) {
	   System.out.println(departureId + "-" + arrivalId);
       String stop1 = index.stopForId.get(GtfsLibrary.convertIdFromString(departureId)).getName();
       String stop2 = index.stopForId.get(GtfsLibrary.convertIdFromString(arrivalId)).getName();
       System.out.println(stop1 + "-" + stop2);
       if (stop1 == null || stop2 == null) return Response.status(Status.NOT_FOUND).entity(MSG_404).build();
       Set<Route> routes = Sets.newHashSet();
       
       for(TripPattern pattern : index.patternForId.values()){
    	   //Riccardo: match intermediate stops 
    	   String s1 = "";
    	   String s2 = "";    	   

    	   System.out.println(pattern.getStops());
		   for(Stop stop : pattern.getStops()){
			   if(stop1.equals(stop.getName())) {
				   s1 = stop.getName();
			   } else if(stop1.equals(s1) && stop2.equals(stop.getName())){
				   s2 = stop.getName();
			   }
			   
			   if(stop1.equals(s1) && stop2.equals(s2)){
	    		   routes.add(pattern.route);
	    		   break;
			   }
		   }
...
//       for (TripPattern pattern : index.patternsForStop.get(stop1)) {
//    	   System.out.println(pattern.route);
//    	   if(pattern.getStop(1).equals(stop2))
//    		   routes.add(pattern.route);
//       }
       return Response.status(Status.OK).entity(RouteShort.list(routes)).build();
   }
   ...
   public Response getPatternsForStop (@PathParam("stopId") String stopIdString) {
    ...
    public Response getStoptimesForStop (@PathParam("stopId") String stopIdString,
                                         ...
                                         @QueryParam("timeRange") @DefaultValue("604800") int timeRange,
...
//        System.out.println("Check service "+index.serviceForId.values());
        return Response.status(Status.OK).entity(index.serviceForId.values()).build();
    }
...
//    	System.out.println(AgencyAndId.convertFromString(serviceId));
        return Response.status(Status.OK).entity(index.serviceForId.get(AgencyAndId.convertFromString(serviceId))).build();
    }
...
//    }
    ...
    //Shuai(16-10-2017) API for frequency
    @GET
    @Path("/frequency/{tripId}")
    public Response getFrequency(@PathParam("tripId") String tripId) {
    	Frequency f = index.frequencyForTripName.get(tripId);
//    	System.out.println("Check frequency " + f);
    	return Response.status(Status.OK).entity(f).build();
    }
    
    //Shuai(24-10-2017) API for calendar_dates
    @GET
    @Path("/calendarDates")
    public Response getCalendarDates() {
//        System.out.println("Check calendar dates "+index.calendarDateForId.values());        
        return Response.status(Status.OK).entity(index.calendarDateForId.values()).build();
    }
    
    @GET
    @Path("/calendarDates/{serviceId}")
    public Response getCalendarDatesForServiceId(@PathParam("serviceId") String serviceId) {
//        System.out.println("Check calendar dates for serviceID "+serviceId);
//        System.out.println(index.calendarDateForId.get(AgencyAndId.convertFromString(serviceId)));        
        return Response.status(Status.OK).entity(index.calendarDateForId.get(AgencyAndId.convertFromString(serviceId))).build();
    }
