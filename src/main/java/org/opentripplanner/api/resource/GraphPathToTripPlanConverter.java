...
public abstract class GraphPathToTripPlanConverter {
    ...
    public static TripPlan generatePlan(List<GraphPath> paths, RoutingRequest request) {
        ...
        //EDIT - Riccardo (07-03-2017): Calculate quality for paths 
       // LOG.info("CO2 Weight = " + Double.toString(request.co2_w) + " " 
        //		+ "Distance Weight = " + Double.toString(request.distance_w) + " " 
        	//	+ "Time Weight = " + Double.toString(request.time_w)); Giovanni
        int[] quality = QualityCalculator.calculatePathsQuality(paths, request);
        //EDIT - Riccardo (07-03-2017): END 
        
        int ii = 0;
        for (GraphPath path : paths) {
        	Itinerary itinerary = generateItinerary(path, request.showIntermediateStops, requestedLocale, request);

            itinerary.quality = quality[ii];
          //  LOG.info("Quality = "+itinerary.quality); //Giovanni
    ...
    public static Itinerary generateItinerary(GraphPath path, boolean showIntermediateStops, Locale requestedLocale) {
    	return generateItinerary(path,showIntermediateStops,requestedLocale,null);
    }
    
    public static Itinerary generateItinerary(GraphPath path, boolean showIntermediateStops, Locale requestedLocale, RoutingRequest request) {
        ...
        //EDIT - Riccardo (31/03/2017): Adding vertices labels to the itineraries, so the client can give them back to us when it booked the trip
        //They will be stored in the database to show previous routes
        JSONArray jsonVertices = new JSONArray();
        for(int j = 0; j < edges.length; j++) {
        	jsonVertices.put(edges[j].getFromVertex().getLabel());
        	if(j == edges.length - 1){
        		jsonVertices.put(edges[j].getToVertex().getLabel());
        	}
        }
        ...
        try {
			if(path.edges.size() != 0) json.put(path.edges.get(path.edges.size() - 1).getToVertex().getName(), jsonVertices);
		} catch (JSONException e) {
			e.printStackTrace();
		} 
        itinerary.setVerticesLabels(json.toString());
...
for(State state: states){
	//System.out.println(state.getVertex().getName() + " : " +state.costTime + "   BackEdge:" + (state.backEdge == null ? "" : (state.backEdge.getName() + "  " + state.backEdge.getClass())));
}
        ...
        for (State[] legStates : legsStates) {
        		Leg leg = generateLeg(graph, legStates, showIntermediateStops, requestedLocale,request);
                //Riccardo: add leg waiting time
//        		if(itinerary.legs.size() == 0) {
//                	leg.waiting = (leg.startTime.getTimeInMillis() - request.getDateTime().getTime()) / 1000;
//                } else {
//                    Leg lastLeg = itinerary.legs.get(itinerary.legs.size() - 1);
//                    leg.waiting = (leg.startTime.getTimeInMillis() - lastLeg.endTime.getTimeInMillis()) / 1000;
//                }
        		itinerary.addLeg(leg);
        		itinerary.waitingTime += leg.waiting;
                itinerary.operationalTime += leg.boarding + leg.transit + leg.alighting;
                itinerary.travelTime += leg.getDurationLong() / 1000;
        }

        itinerary.legs.get(itinerary.legs.size() - 1).finalDestination = 1;
        ...
        //LOG.info("The start time of the itinerary is "+itinerary.startTime.getTimeInMillis()/(60*1000)); Giovanni
        itinerary.endTime = makeCalendar(lastState);
        Leg firstLeg = itinerary.legs.get(0);
    	//EDIT - Riccardo (31/05/2017): add alight on last
        itinerary.elapsedTime = itinerary.waitingTime + itinerary.operationalTime + itinerary.travelTime;
        itinerary.duration = ( (itinerary.endTime.getTimeInMillis() - firstLeg.startTime.getTimeInMillis() ) / 1000 ) + firstLeg.boarding;
//        itinerary.waitingTime = itinerary.elapsedTime - itinerary.travelTime - itinerary.operationalTime;
        
//        System.out.println("Operational time: " + (double) itinerary.operationalTime);
//        System.out.println("Elapsed time: " + (double) itinerary.elapsedTime);
//        System.out.println("Travel time: " + (double) itinerary.travelTime);
//        System.out.println("Waiting time: " + (double) itinerary.waitingTime);

//    	Calendar c = Calendar.getInstance();
//    	c.setTimeInMillis(itinerary.endTime.getTimeInMillis());
//        System.out.print(c.getTime() + " ");	     
//        c.setTimeInMillis(firstLeg.startTime.getTimeInMillis());
//    	System.out.println(c.getTime());
//    	System.out.println(itinerary.duration /3600+ " " + itinerary.elapsedTime/3600);
        //EDIT - Riccardo (31/05/2017): END
        
        itinerary.CO2 = lastState.CO2;
        itinerary.costCO2 = lastState.costCO2;
        itinerary.distance = lastState.distance;
        itinerary.costDistance = lastState.costDistance;

        //itinerary.totalcost1 = lastState.weight - states[0].weight; same as totalcost
        //RoutingRequest option = lastState.getOptions();
        //itinerary.totalcost = option.co2_w * lastState.costCO2 + option.distance_w * lastState.costDistance + option.time_w * lastState.costTime;
        //LOG.info("Multi = {}, TotalCost = {}, CO2 = {}, dis = {}, time = {}",itinerary.totalcost_multiobjective, itinerary.totalcost,lastState.costCO2,lastState.costDistance,lastState.costTime); Giovanni
        
        //Riccardo edited costs
        itinerary.waitingCost = itinerary.waitingTime * request.cost_per_hour / 3600;
        itinerary.operationalCost = itinerary.operationalTime * request.cost_per_hour / 3600;
        itinerary.travelTimeCost = itinerary.travelTime * request.cost_per_hour / 3600;
        itinerary.travelCost = itinerary.totalcostSHIP + itinerary.totalcostTRAIN + itinerary.totalcostTRUCK;
        itinerary.costTime = itinerary.travelTimeCost + itinerary.operationalCost + itinerary.waitingCost;
        itinerary.totalcost = itinerary.costTime + itinerary.costDistance + itinerary.costCO2;

//        System.out.println(itinerary.elapsedTime/3600 + " " + itinerary.travelTime/3600 + " " + itinerary.waitingTime/3600 + " " + itinerary.operationalTime/3600);
//        System.out.println(itinerary.totalcost + "  " + itinerary.travelCost + " " + itinerary.waitingCost + " " + itinerary.operationalCost);
        
        //LOG.info("W1 = {}, W2 = {}, W3 = {}",itinerary.WAIT1,itinerary.WAIT2,itinerary.WAIT3 ); Giovanni
        
        calculateTimes(itinerary, states);

        calculateElevations(itinerary, edges);

        //EDIT - Riccardo (28/03/2017): mark itinerary from database 
        if(path.isFromDatabase()){
        	itinerary.fromDatabase = true;
        }
...
//System.out.println("legIndexPairs: " +legIndexPairs[1]);
...
//System.out.println("backMode:" +backMode + "                forwardMode:" +forwardMode);
            if(backMode.toString().equals("WALK") || forwardMode.toString().equals("WALK") ) continue; //EDIT - Riccardo (28/03/2017): Avoid walking leg
        ...
        // Final leg
        legsIndexes.add(legIndexPairs);
//System.out.println("legsIndexes size: "+legsIndexes.size());
        State[][] legsStates = new State[legsIndexes.size()][];

        // Fill the two-dimensional array with states
        for (int i = 0; i < legsStates.length; i++) {
//System.out.println(legsIndexes.get(i).length);
//System.out.println(legsIndexes.get(i)[0] + " " +legsIndexes.get(i)[1]);
     ...
     */
    private static Leg generateLeg(Graph graph, State[] states, boolean showIntermediateStops, Locale requestedLocale, RoutingRequest request) {
    	
        Leg leg = new Leg();

        Edge[] edges = new Edge[states.length - 1];
        /*EDIT-Yuanyuan(28/06/2018): Add the transit time
        Trip trip = states[0].getTripTimes().trip;
        leg.boarding = GTFSExtraDataManager.getBoardingTime(trip, request);
        leg.alighting = GTFSExtraDataManager.getAlightingTime(trip, request);*/
        /*EDIT-Yuanyuan(23/06/2018): Add the trip_headsign from trips.txt to leg representing  slow/fast steaming*/
       // Trip trip = states[0].getTripTimes().trip;
       // leg.steamingType = trip.getTripHeadsign(); // TO BE MODIFIED
        
        leg.startTime = makeCalendar(states[0]);
        leg.endTime = makeCalendar(states[states.length - 1]);
        /*leg.CO2 = states[states.length - 1].CO2; will be accumulated from the beginning of the itinerary
        leg.costCO2 = states[states.length - 1].costCO2;
        leg.totalcost = states[states.length - 1].weight;
        leg.costdistance = states[states.length - 1].costDistance;
        leg.costtime = states[states.length - 1].costTime;*/
        
//System.out.println("1st state cost CO2=" + states[0].costCO2 + " Distance=" + states[0].costDistance + " Time=" + states[0].costTime);
//System.out.println("2nd state cost CO2=" + states[states.length - 1].costCO2 + " Distance=" + states[states.length - 1].costDistance + " Time=" + states[states.length - 1].costTime);
        //Giovanni TODO: togliere i commenti ai log sottostanti
		//LOG.info("The cost of each leg is "+leg.totalcost);
        //LOG.info("leg cost co2={}, distance={}, time={} ",leg.costCO2, leg.costdistance, leg.costtime);
 
        // Calculate leg distance and fill array of edges
//        leg.distance = 0.0;
        for (int i = 0; i < edges.length; i++) {
            edges[i] = states[i + 1].getBackEdge();
            //leg.distance += edges[i].getDistanceInMeters(); //PHUONG: in meter
        }
        leg.distance = (states[states.length - 1].distance - states[0].distance) * 1000;
...
//        System.out.println( leg.from.name + " " + leg.to.name);
//        for(int i = 0; i < states.length; i++){
//        	System.out.println(states[i].boarding + " " + states[i].transit + " " + states[i].alighting);
//        }
        
//        System.out.println();
//        System.out.println(leg.from.name + " " + states[0].getOptions().from.name);
      //Giovanni (23-11-2017) : capacità states[0].capacity creando leg.capacity salvo nella leg lo stato 
        leg.capacity = states[0].capacity;
        //Giovanni (23-11-2017): END
        
        //Riccardo (04-09-2018): adding operational time on legs
        leg.boarding = states[0].boarding;
        	//LOG.info("The board is "+leg.boarding); Giovanni
        leg.alighting = states[states.length - 1].alighting;
        leg.waiting = states[0].wait;
//        leg.endTime.setTimeInMillis(leg.endTime.getTimeInMillis() - leg.alighting * 1000);
        //Riccardo (07-06-2017): END
        
        leg.CO2 = states[states.length - 1].CO2 - states[0].CO2;
        leg.costCO2 = states[states.length - 1].costCO2 - states[0].costCO2;
        leg.totalcost_multiobjective = states[states.length - 1].weight - states[0].weight;
        leg.costdistance = states[states.length - 1].costDistance - states[0].costDistance;
        leg.costtime = request.cost_per_hour * (leg.endTime.getTimeInMillis() - leg.startTime.getTimeInMillis()) / 3600000;    
        leg.totalcost = leg.costCO2 + leg.costdistance + leg.costtime;
        ...
        //Giovanni GTFSExtraDataManager.getCapacity();
        //Giovanni default capacity 
        
        if(request!=null) {
        	if(leg.isTRUCKLeg()) {
        		leg.assignedcontainers = request.capacityRequest;
        	}
        }
        
//        if(leg.isSHIPLeg()) System.out.println(leg.distance);
