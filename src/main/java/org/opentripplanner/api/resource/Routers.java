...
//    @RolesAllowed({ "ROUTERS" })
    ...
    public Response putGraphId(@PathParam("routerId") String routerId,
    	...
    	//Shuai(26-06-2018) rebuild the graph.obj
    	GraphRebuilder.rebuild(otpServer, routerId);
        ...
        if (graphService.getRouterIds().contains(routerId)) {
            boolean success = graphService.reloadGraph(routerId, preEvict, true);
            System.out.println("reload success: " + success);
            if (success)
