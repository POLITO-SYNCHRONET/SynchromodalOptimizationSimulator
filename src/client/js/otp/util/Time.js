...
otp.util.Time = {
    ...
    secsToHrMinSecWithSeparator : function(secs, separator) {
        var hrs = Math.floor(secs / 3600);
        var mins = Math.floor(secs / 60) % 60;
        var secs = secs % 60;

        var str = (hrs > 0 ? ("" + (hrs < 10 ? ("0" + hrs) : hrs) + separator) : "00" + separator) 
        		+ (mins > 0 ? ("" + (mins < 10 ? ("0" + mins) : mins) + separator) : "00"+ separator) 
        		+ (secs > 0 ? ("" + (secs < 10 ? ("0" + secs) : secs)) : "00");
        return str;
    },
    
	secsToDhms: function(timeStart, timeEnd, separator){
		var oneDay = 24*60*60;
		console.log('timestart:' + timeStart + ' timeend: '+ timeEnd + ' oneDay:' + oneDay);
		if(timeEnd < oneDay){
			console.log(this.secsToHrMinSecWithSeparator(timeEnd, separator) + '+0day');
			return  this.secsToHrMinSecWithSeparator(timeEnd, separator) + '+0';
		}
		else{
			var durationInDays = Math.floor(timeEnd / oneDay); //number of whole days in between
			var last = timeEnd % oneDay; //seconds left in the last day
//			console.log('days: ' + durationInDays + ' left:' + last);
			var durationString = this.secsToHrMinSecWithSeparator(last, separator);
			durationString += '+' +  durationInDays;
			console.log(durationString  + 'days');
...
//    	console.log("format Str:" + formatStr);
//    	console.log(otp.config.timeOffset);
