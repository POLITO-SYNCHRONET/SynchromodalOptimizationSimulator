...
otp.core.Webapp = otp.Class({
    ...
    initialize : function() {
        ...
        //Shuai(02-03-2018) User's ID and role are given by SOAG's platform through url parameters. 
        console.log(this.urlParams);
        //Shuai(01-03-2018) user widget 
        if('user' in this.urlParams)
        	this.username = this.urlParams['user'];
        if('role' in this.urlParams)
        	this.role = this.urlParams['role'];
        if('umcsession' in this.urlParams)
        	this.umcSession = this.urlParams['umcsession'];
        console.log(this.username);
        console.log(this.role);
        console.log(this.umcSession);
        
        //TODO user authentication
        
        //TODO role should not be null
        //check if the user has already stored in our database. If no, add user as well as the default configuration for the user
        if(this.username != null && this.role != null){
        	var url = otp.config.hostname + '/otp/routers/' + this.username  + '/user/userCheck/?userrole=' + this.role;
	        $.ajax(url, {
				type: 'GET',
				dataType: 'JSON',
        ...
        if(otp.config.infoWidgets !== undefined && otp.config.infoWidgets.length > 0) {
          ...
          //Yuanyuan (06-07-2017)
            //User Experience
            var userExperienceId = 'otp-userExperienceWidget';
            this.userExperienceWidget = new otp.widgets.userExperienceWidget(userExperienceId, this, null);
            this.userExperienceWidget.doAfterLayout();
//            console.log(this.userExperienceWidget.title);
            $("<li id='otp-userExperience'><a href='#'>"+this.userExperienceWidget.title+"</a></li>").appendTo(ul).click(function(e) {
                e.preventDefault();
                var widget = this_.userExperienceWidget;
                if(!widget.isOpen) widget.show();
                widget.bringToFront();
            });

           //Shuai(03-05-2018) new user bar
            if(this.username != null && this.role != null){
	            var popup = new otp.widgets.UserPopupMenu(this, this.username, this.role);
	            $("<li id='otp-username'><a href='#'>"+ this.username +"</a></li>").appendTo(ul).click(function(e) {
	            	popup.show();
	            });
            }
            else{
...
//            	this.username="default";
                var loginID = "otp-loginWidget";
                this.loginWidget = new otp.widgets.LoginWidget(loginID, this, null);
                this.loginWidget.doAfterLayout();
//                console.log(this.loginWidget.title);
                $("<li id='otp-login-button'><a href='#'>"+this.loginWidget.title+"</a></li>").appendTo(ul).click(function(e) {
                    e.preventDefault();
                    var widget = this_.loginWidget;
                    if(!widget.isOpen) widget.show();
                    widget.bringToFront();
                });
                
                //register widget
                var registerID = "otp-registerWidget";
                this.registerWidget = new otp.widgets.RegisterWidget(registerID, this, null);
                this.registerWidget.doAfterLayout();
                //End Shuai
            }
        }
        ...
        if(otp.config.modules) {
            ...
            for(var i=0; i<otp.config.modules.length; i++) {
                ...
                //Shuai(03-05-2018) pass username to each module
                if(this.username != null && this.role != null){
                	options['username'] = this.username;
                	options['userrole'] = this.role;
                }
...
//        }));
        ...
        //Shuai(06-03-2018) add a spinner at the start up of otp to indicate the loading of the data and initialization of otp
        $(Mustache.render(otp.templates.img, {
            src: 'images/loading2.gif',
            wrapDiv: true,
            divId: 'otp-startupSpinner',
        })).appendTo($('body'));
    ...
    activateModule : function(module) {
    	console.log('activateModule: ' + module.moduleName);
        module.activate();
    ...
    moduleSelected : function(module) {
    	console.log('moduleSelected: ' + module.moduleName);
        module.selected();
