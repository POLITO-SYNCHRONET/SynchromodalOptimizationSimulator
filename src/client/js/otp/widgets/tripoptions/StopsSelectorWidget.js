...
otp.widgets.StopsSelectorWidget =
    ...
    initialize : function(id, routesControl, name) {
        ...
        ich['otp-tripOptions-stopsSelector']({
            widgetId : this.id,
            name : this.name,
            //TRANSLATORS: All public transport stops. Shown in
            //Banned stops widget
            allRoutes : _tr("All Stops"),
            //TRANSLATORS: save banned public transport stops
            save : _tr("Save"),
            //TRANSLATORS: Close banned public transport stops
            //widget
            close : _tr("Close")
        }).appendTo(this.$());
        ...
        $('#'+this.id+'-saveButton').button().click(function() {
            ...
            for(var i = 0; i < this_.selectedRouteIds.length; i++) {
                var route = this_.indexApi.stops[this_.selectedRouteIds[i]];
		        ...
		        //format expected: agency:stopid, so, in our case, using :
                paramStr += route.id.replace(":", "") + (i < this_.selectedRouteIds.length-1 ? ',' : '');
                displayStr += (route.name) + (i < this_.selectedRouteIds.length-1 ? ', ' : '');
    ...
    updateRouteList : function() {
        ...
        this.indexApi.loadStops_PHUONG(this, function() {
