...
otp.widgets.BikeTrianglePanel = new otp.Class({
	...
	triangleCO2Factor:    null,
    triangleTimeFactor:   null,
    triangleDistanceFactor:  null,
    ...
    //for speed
    quickName   : _tr("Emission(CO2)"),
    //TRANSLATORS: Optimization for bicycle shown in bike triangle. Optimized
    //for flat terrain
    flatName    : _tr("Elapse Time(Total)"),
    //TRANSLATORS: Optimization for bicycle shown in bike triangle. Optimized
...
//console.log($(this.div).width());
        ...
        var flatSym  = _tr("D"); //locale.bikeTriangle.flatSym;
        
        var quickFill = "#ffb2b2";
        var quickFill2 = "#f27979";
        //TRANSLATORS: First letter of Quick
        var quickSym  = _tr("E"); //locale.bikeTriangle.quickSym;
        ...
        //Shuai (21-12-2017) fix the problem that sometimes triangle texts are not replaced in the right position
        if (labelT.node.childNodes[0].attributes[0].name == "dy") {
        	labelT.node.childNodes[0].attributes[0].value = 6;        
          }
        if (labelH.node.childNodes[0].attributes[0].name == "dy") {
        	labelH.node.childNodes[0].attributes[0].value = 6;        
          }

        if (labelS.node.childNodes[0].attributes[0].name == "dy") {
        	labelS.node.childNodes[0].attributes[0].value = 6;        
          }
        ...
        up = function () {  console.log(time);
            ...
            this_.CO2Factor = time;
            this_.TimeFactor = topo;
            this_.DistanceFactor = safety;
            if(this_.onChanged && typeof(this_.onChanged) === "function") {
            	console.log(this_.onChanged);
                this_.onChanged();
                console.log(time);
            } else
            	console.log(typeof(this_.onChanged));
        };
    ...
    setValues : function(quick, flat, safe) {
        this.CO2Factor = quick;
        this.TimeFactor = flat;
        this.DistanceFactor = safe;
    ...
    getFormData : function() {
        return {
                co2_w				   : this.CO2Factor,
                time_w			       : this.TimeFactor,
                distance_w			   : this.DistanceFactor
        }
    },
