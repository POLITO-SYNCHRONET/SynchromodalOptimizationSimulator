...
//        	}
    ...
    renderHeaderContent : function(itin, index, parentDiv) {
    	...
    	//Shuai (04-2017) manage the trip with the 2nd destination
    	var has2ndDest = false;
    	
    	parentDiv.empty();
        //parentDiv.append('<div class="otp-itinsAccord-header-number">'+(index+1)+'.</div>');
        
    	var div = $('<div style="position: relative; height: 20px; margin-top: 5px;"></div>').appendTo(parentDiv);
    	...
    	//check if there is the 2nd destination
        if(this.hasMultiDestId(itin, '2'))
        	has2ndDest = true;
        
        //common part of the itinerary for the 2 destinations
        var commonDiv;
        
        if(has2ndDest){
        	commonDiv = $('<div style="position: relative; height: 20px;"></div>').prependTo(parentDiv);
        	commonDiv.append('<div class="otp-itinsAccord-header-number">'+(index+1)+'.</div>');
        	
        	div.append('<div class="otp-itinsAccord-header-number" style="font-size: 1em">'+(index+1.1)+'.</div>');
        	
        	var timeStr = otp.util.Time.formatItinTime(itin.getStartTime(), otp.config.locale.time.time_format);
        	//check if there is common parts
        	if(this.hasMultiDestId(itin, '0')){
	            commonDiv.append('<div class="otp-itinsAccord-header-time" style="left: '+(leftPx-32)+'px;">' + timeStr + '</div>');
        	}
        	else{
        		commonDiv.append('<div style="position: relative; width: 150px; left: '+(leftPx-32)+'px;">No Common Parts</div>');
                div.append('<div class="otp-itinsAccord-header-time" style="left: '+(leftPx-32)+'px;">' + timeStr + '</div>');
        	}
        }
        else{
        	div.append('<div class="otp-itinsAccord-header-number" style="font-size: 1em">'+(index+1)+'.</div>');
        ...
        if(has2ndDest){
        	...
        	div2.append('<div class="otp-itinsAccord-header-number" style="font-size: 1em">'+(index+1.2)+'.</div>');
        ...
        for(var l=0; l<itin.itinData.legs.length; l++) {
        	...
        	//check which destination the leg belongs to
	        	if(leg.multiDestinationId == '0'){   	
		            segment.appendTo(commonDiv); 
	        	}
	        	else if(leg.multiDestinationId == '1'){
	        		segment.appendTo(div);
	            }
	        	else{
	        		segment.appendTo(div2);
	            } 
        	}
        	else{
        		segment.appendTo(div);
...
=======
            					'arrival': queryParams.toPlace + (queryParams.to2ndPlace == null ? '' : '-' + queryParams.to2ndPlace),
>>>>>>> 1ae211198770110778458af88a65db259060cd19
    ...
    hasMultiDestId: function(itin, id){
        for(var l=0; l<itin.itinData.legs.length; l++) {
        	if(itin.itinData.legs[l].multiDestinationId == id)
        		return true;
        }
        return false;
    }
