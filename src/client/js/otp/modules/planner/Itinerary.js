...
otp.modules.planner.Itinerary = otp.Class({
    ...
    //Shuai (09-06-2017) calculate the start/end time concerning boarding/alighting time
    getStartTime : function() {
//    	console.log('start:' + this.itinData.legs[0].startTime + ' -> ' + (this.itinData.legs[0].startTime - this.itinData.legs[0].boarding))
//        return (this.itinData.legs[0].startTime - this.itinData.legs[0].boarding * 1000);      
//    	return this.itinData.startTime;
        
        var startTime = null;
    	for(var j=0; j<this.itinData.legs.length; j++){
    		var leg  = this.itinData.legs[j];
    		if(startTime == null)
    			startTime = leg.startTime - leg.boarding * 1000;
    		else{
    			if(startTime > (leg.startTime - leg.boarding * 1000))
    				startTime = leg.startTime - leg.boarding * 1000;
    		}
    	}
    	return startTime;
...
//    	return this.itinData.endTime;
    	var endTime = null;
    	for(var j=0; j<this.itinData.legs.length; j++){
    		var leg  = this.itinData.legs[j];
    		if(endTime == null)
    			endTime = leg.endTime + leg.alighting * 1000;
    		else{
    			if(endTime < (leg.endTime + leg.alighting * 1000))
    				endTime = leg.endTime + leg.alighting * 1000;
    		}
    	}
//    	console.log('end:' + this.itinData.legs[0].endTime + ' -> ' + endTime)
    	return endTime;
    },
    ...
    getCO2Str : function() {
		...
		return this.getCO2().toFixed(2) + " kg";
		/* var CO2_info = {
               
                'price': this.getCO2(),
            }
        return _tr('%(price)s', CO2_info);*/
...
//        return otp.util.Time.secsToHrMin( (this.getEndTime() - this.getStartTime())/1000.0 );
    	return otp.util.Time.secsToHrMin(this.itinData.duration);
    },
    ...
    getTransportMode : function() {
		...
		var segments="";
		if (this.itinData.haveTRUCK) 
		{	if (this.itinData.haveTRAIN)
			{
				if (this.itinData.haveSHIP) segments = "TRUCK, TRAIN, SHIP";
				else segments = "TRUCK, TRAIN";
			}
			else			
			{
				if (this.itinData.haveSHIP) segments = "TRUCK, SHIP";
				else segments = "TRUCK";
			}
		}
		else{
			if (this.itinData.haveTRAIN)
			{
				if (this.itinData.haveSHIP) segments = "TRAIN, SHIP";
				else segments = "TRAIN";
			}
			else			
			{
				if (this.itinData.haveSHIP) segments = "SHIP";
			}
		}
		return segments;
    },
    ...
    getHtmlNarrative : function() {
        ...
        html += '<div class="otp-itinTripSummaryLabel">' + _tr('Departure') + '</div><div class="otp-itinTripSummaryText">'+this.getStartTimeStr()+'</div>';
        html += '<div class="otp-itinTripSummaryLabel">' + _tr('Duration') + '</div><div class="otp-itinTripSummaryText">'+this.getDurationStr()+'</div>';
        if(this.hasTransit) {
            html += '<div class="otp-itinTripSummaryLabel">' + _tr('Number of transfers') + '</div><div class="otp-itinTripSummaryText">'+this.itinData.transfers+'</div>';
    ...
    //Shuai (13-07-2017) calculate the internal transfers of the itinerary
...
//    	for(var i=0; i < this.itinData.legs.length; i++){
//    		var leg = this.itinData.legs[i];
//    		if(!(leg.destinationId in destVisited))
//    			destVisited[leg.destinationId] = 0;
//    		if(!(leg.previousDestinationId in prevVisited))
//    			prevVisited[leg.previousDestinationId] = 0;
//    	}
//    	if(destVisited.length == 1){
//    		transfers = this.itinData.legs.length-1;
//    	}
//    	else{
//    		prevVisited['0'] = 1;
//    		for(var i=0; i < this.itinData.legs.length; i++){
//    			var leg = this.itinData.legs[i];
//    			if(prevVisited[leg.previousDestinationId] == 0){
//    				transfers += 1;
//    				prevVisited[leg.previousDestinationId] = 1;
//    				destVisited[leg.destinationId] = 1;
//    			}
//    			else{
//    				if(destVisited[leg.destinationId] == 0){
//    					destVisited[leg.destinationId] = 1;
//    				}
//    				else{
//    					transfers += 1;
...
//    	return transfers;
    	...
    	//Shuai (11-05-2018) probably doing a simple way: just check the ID of all the stops and count how many stops internally.
    	var stopsVisited = [];
    	for(var i=0; i < this.itinData.legs.length; i++){
    		var leg = this.itinData.legs[i];
    		var stop = leg.from.name.toUpperCase();
    		if(!stopsVisited.includes(stop))
    			stopsVisited.push(stop);
    		stop = leg.to.name.toUpperCase();
    		if(!stopsVisited.includes(stop))
    			stopsVisited.push(stop);
    	}
    	console.log(stopsVisited.length);
    	return stopsVisited.length - 2; //ignore start/end stops
    }, 
});
