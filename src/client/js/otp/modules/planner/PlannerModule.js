...
otp.modules.planner.defaultQueryParams = {
    ...
    timeArrive                      : moment().format(otp.config.locale.time.time_format), //PHUONG
    dateArrive                      : moment().format(otp.config.locale.time.date_format), //PHUONG
	timeArriveVisible	            : false, //PHUONG
    arriveBy                        : false,
    wheelchair                      : false,
	sortResultType                  :0, //PHUONG
    mode                            : "TRANSIT,WALK",
    ...
    time_w			                : 0.333,
    distance_w			  		    : 0.334,
	slowSteaming					: false, //PHUONG
	KPIimportance					: false, //PHUONG
	co2_cost_per_kg					: 0.15, //PHUONG
	cost_per_hour					: 2, //PHUONG
	cost_distance_per_km_truck				: 0.3, //PHUONG
	cost_distance_per_km_train				: 0.4, //PHUONG
	cost_distance_per_km_ship				:     0.001, //PHUONG
	...
	co2_per_km_ship: 80,
	co2_per_km_ship_Slow: 60,
	co2_per_km_ship_Fast: 100,
	boarding_truck: 60,
	boarding_train: 120,
	boarding_ship: 360,
	alighting_truck: 60,
	alighting_train: 120,
	alighting_ship: 360,
	boarding_ship_roro: 180,
	alighting_ship_roro: 180,
	truck_speed: 0,
	capacity_train: 200,
	capacity_ship: 1000,		
}
...
otp.modules.planner.PlannerModule =
    ...
    */
    ...
    otherEndName : [],  //index 0 and 1 empty
    otherEndLatLng: [],
    
    //recording the arrival time/date and capacity for each destination
    otherDateArrive : [],
    otherTimeArrive : [],
    otherTimeArriveVisible : [],
    otherCapacity: [],
    
    riskAnalysis: false,
    
    driverRestTime: false,
    ...
    error_messages : {
        500 : _tr("We're sorry. The SYNCHRONET planner is temporarily unavailable. Please try again later."),
        503 : _tr("We're sorry. The SYNCHRONET planner is temporarily unavailable. Please try again later."),
        ...
        340 : _tr("The SYNCHRONET planner is unsure of the location you want to start from. Please select from the following options, or be more specific."),
        350 : _tr("The SYNCHRONET planner is unsure of the destination you want to go to. Please select from the following options, or be more specific."),
        ...
        371 : _tr("The values of distanceFactor, TimeFactor, and CO2Factor must sum to 1"),
        372 : _tr("If distanceFactor, TimeFactor, and CO2Factor are provided, OptimizeType must be TRIANGLE"),
        373 : _tr("If OptimizeType is TRIANGLE, distanceFactor, TimeFactor, and CO2Factor must be set"),
...
//            this_.setApproximateStartEnd(latlng, 'start');
//        });
//        //TRANSLATORS: Context menu
//        this.webapp.map.addContextMenuItem(_tr("Set as End Location"), function(latlng) {
//            this_.setApproximateStartEnd(latlng, 'end');
//        });
    },
...
//        	this.setApproximateStartEnd(new L.LatLng(event.latlng.lat, event.latlng.lng), 'start');
//        }
//
//        else if(this.endLatLng == null) {
//        	this.setApproximateStartEnd(new L.LatLng(event.latlng.lat, event.latlng.lng), 'end');
//        }
    },
    ...
    setStartPoint : function(latlng, update, name) {
        ...
        if(this.startMarker == null) {
            this.startMarker = new L.Marker(this.startLatLng, {icon: this.icons.startFlag, draggable: true}); //PHUONGL create the marker position for the "Start flag"
            ...
            this.startMarker.setLatLng(latlng); //PHUONG: Changes the marker position to the given point.
        }
	...
	setEndPoint : function(latlng, update, name, index) {
		...
		//Shuai (28-04-2017) set markers for other destination
		else{
			console.log(index);
			this.otherEndName[index] = (typeof name !== 'undefined') ? name : null;
	        this.otherEndLatLng[index] = latlng;
	        if(this.otherEndMarker[index] == null) {
	            this.otherEndMarker[index] = new L.Marker(this.otherEndLatLng[index], {icon: this.icons.endFlag, draggable: true});
	            //TRANSLATORS: shown in a popup on last point of a path in a map
	            this.otherEndMarker[index].bindPopup('<strong>' + _tr('Destination') + '</strong>');
	            this.otherEndMarker[index].on('dragend', $.proxy(function() {
	                this.webapp.hideSplash();
	                this.setEndPoint(this.otherEndMarker[index].getLatLng(), false);
	                this.invokeHandlers("endChanged" + index, [this.otherEndLatLng[index]]);
	                if(typeof this.userPlanTripStart == 'function') this.userPlanTripStart();
	                this.planTripFunction.apply(this);//this_.planTrip();
	            }, this));
	            this.markerLayer.addLayer(this.otherEndMarker[index]);
	        }
	        else { // marker already exists
	            this.otherEndMarker[index].setLatLng(latlng);
	        }
	
	        this.invokeHandlers("endChanged" + index, [latlng, name, index]);
    ...
    getStartOTPString : function() { //PHUONG: return "name::place" string, where "place" is a latitude,longitude string
    ...
    restoreTrip : function(queryParams) {
        ...
        this.planTripFunction.call(this, queryParams); //PHUONG = planTrip();
    },
    ...
    planTrip : function(existingQueryParams, apiMethod) {
        ...
        {        	
        	//we have only 1 check. If genericCall widget exists, we use only one-line query for testing.
        	if(!this.genericTool){

	        	if(this.startLatLng == null || this.endLatLng == null) {
					var msg = _tr("Origin/Destination is unknown. Can you be a bit more descriptive?");
					otp.widgets.Dialogs.showOkDialog(msg, _tr('Not enough input'));
					// TODO: alert user
	                return;
	            }
	        	
				 if (this.sortResultType == null)
				 {
					var msg = _tr("How do you want to sort the results ?. Can you be a bit more descriptive?");
					otp.widgets.Dialogs.showOkDialog(msg, _tr('Not enough input'));
					return;
				 }
	            ...
	            console.log(this);
				if (this.timeArriveVisible)
				{
					queryParams = {
						fromPlace: this.getStartOTPString(), //PHUONG: "name::place" string, where "place" is a latitude,longitude string or a vertex ID.
						toPlace: this.getEndOTPString(),
						time : (this.time) ? otp.util.Time.correctAmPmTimeString(this.time) : moment().format("h:mma"),
						//time : (this.time) ? moment(this.time).add("s", addToStart).format("h:mma") : moment().add("s", addToStart).format("h:mma"),
						date : (this.date) ? moment(this.date, otp.config.locale.time.date_format).format("MM-DD-YYYY") : moment().format("MM-DD-YYYY"),
						mode: (this.mode == null) ? "TRANSIT,WALK" : this.mode,
						timeArriveVisible: this.timeArriveVisible,
						maxWalkDistance: this.maxWalkDistance,
						sortResultType :this.sortResultType,
						timeArrive : (this.timeArrive) ? otp.util.Time.correctAmPmTimeString(this.timeArrive) : moment().format("h:mma"),
						//time : (this.time) ? moment(this.time).add("s", addToStart).format("h:mma") : moment().add("s", addToStart).format("h:mma"),
						dateArrive : (this.dateArrive) ? moment(this.dateArrive, otp.config.locale.time.date_format).format("MM-DD-YYYY") : moment().format("MM-DD-YYYY"),
						slowSteaming					: this.slowSteaming,
						KPIimportance					: this.KPIimportance,
						co2_cost_per_kg					: this.co2_cost_per_kg,	
						cost_per_hour					: this.cost_per_hour,
						cost_distance_per_km_truck	    : this.cost_distance_per_km_truck,
						cost_distance_per_km_train	    : this.cost_distance_per_km_train,
						cost_distance_per_km_ship	    : this.cost_distance_per_km_ship,
						co2_w						    : this.co2_w,
						time_w			                : this.time_w,
						distance_w			  		    : this.distance_w,
						KRI_time_w						: this.KRI_time_w,
						KRI_flexibility_w				: this.KRI_flexibility_w,
						KRI_safety_w					: this.KRI_safety_w,
						KRI_cost_w						: this.KRI_cost_w,
						
						co2_per_km_truck: this.co2_per_km_truck,
						co2_per_km_train: this.co2_per_km_train,
						co2_per_km_ship: this.co2_per_km_ship,
						co2_per_km_ship_Slow: this.co2_per_km_ship_Slow,
						co2_per_km_ship_Fast: this.co2_per_km_ship_Fast,
						boarding_truck: this.boarding_truck,
						boarding_train: this.boarding_train,
						boarding_ship: this.boarding_ship,
						alighting_truck: this.alighting_truck,
						alighting_train: this.alighting_train,
						alighting_ship: this.alighting_ship,
						boarding_ship_roro: this.boarding_ship_roro,
						alighting_ship_roro: this.alighting_ship_roro,
						truck_speed: this.truck_speed,
						capacity_train: this.capacity_train,
						capacity_ship: this.capacity_ship,
					};
				}
				else
				{
	            ...
	            if(this.ForcedRoutes !== null) _.extend(queryParams, { ForcedRoutes : this.ForcedRoutes } );
	            ...
	            if(this.numContainer){
	            	queryParams.capacityRequest = this.numContainer;
	            }
	            
	            //Shuai(18-01-2018) 
	            if(this.riskAnalysis)
	            	queryParams.riskAnalysis = true;
	            else
	            	queryParams.riskAnalysis = false;
	            
	            //Shuai(13-07-2018) 
	            if(this.driverRestTime)
	            	queryParams.driverRestTime = true;
	            else
	            	queryParams.driverRestTime = false;
	            
	            //Shuai (28-04-2017) to other destinations
	            if(this.otherEndName.length > 2){
	            	queryParams.otherEndNameStr = '';
	            	for(var l=2; l<this.otherEndName.length; l++)
	            		queryParams.otherEndNameStr += this.otherEndName[l] + '::' + this.otherEndLatLng[l].lat + ',' + this.otherEndLatLng[l].lng + (l+1 < this.otherEndName.length ? '---' : '');
	            	
	            	//to be removed
	            	queryParams.to2ndPlace = queryParams.otherEndNameStr;
	            }
	            
	            
	            //Shuai (07-06-2017) arrive time for other destinations
	            if(this.otherTimeArriveVisible.length != 0){
	            	queryParams.otherTimeArriveVisible = '';
	            	queryParams.otherTime = '';
	            	queryParams.otherDate = '';
	            	for(var i=0; i < this.otherTimeArriveVisible.length; i++){
	            		queryParams.otherTimeArriveVisible += this.otherTimeArriveVisible[i] + (i+1 < this.otherTimeArriveVisible.length ? '---' : '');
	            		queryParams.otherDate += moment(this.otherDateArrive[i], otp.config.locale.time.date_format).format("MM-DD-YYYY") + (i+1 < this.otherTimeArriveVisible.length ? '---' : '');
	            		queryParams.otherTime += otp.util.Time.correctAmPmTimeString(this.otherTimeArrive[i]) + (i+1 < this.otherTimeArriveVisible.length ? '---' : '');
	            	}
	            }
	            
	            
	            //Shuai(20-03-2018) number of container to ship for each other destinations.
	            if(this.otherCapacity.length != 0){
	            	queryParams.otherCapacityRequest = '';
	            	for(var i=0; i < this.otherCapacity.length; i++){
	            		queryParams.otherCapacityRequest += ((this.otherCapacity[i] == '') ? 0 : this.otherCapacity[i]) + (i+1 < this.otherCapacity.length ? '---' : '');
	            	}
//	            	console.log(queryParams.otherCapacityRequest);
	            }
	            
	            
	            //Shuai (03-05-2017) define the intermediate stop
	            if(this.passBy)
	            	queryParams.passBy = this.passBy;
	      
	            //Shuai (06-06-2017)
	            if(this.BannedStops)
	            	queryParams.BannedStops = this.BannedStops;
	            
	            //Shuai (04-04-2017) send also the user info
	            if(this.webapp.loginWidget != null && this.webapp.loginWidget.logged_in){
	            	queryParams.username = $('#otp-username :first-child').html();
	            }
//	        }
		        $('#otp-spinner').show();
    ...
    planTripRequest : function(url, queryParams, successCallback) {
        ...
        //test for one-line generic call
    	if(this.genericTool){
    		var params = [];
    		var query = $('#otp-planner-optionsWidget-genericCall').val();
    		url += query;
                ...
                success: function(data) {
                    ...
                    if(data.plan) {
                        ...
                        if(typeof this_.preprocessPlan == 'function') {
                            this_.preprocessPlan(data.plan, params, function() {
                                this_.planReceived(data.plan, url, params, successCallback);
                            });
                        }
                        else {
                            this_.planReceived(data.plan, url, params, successCallback);
        ...
        this.currentRequest = $.ajax(url, {
            ...
            beforeSend: function(){
            	//Shuai(16-04-2018) input check
            	var divs;
            	
            	divs = $('#otp-planner-optionsWidget-capacityWidget-input').parent().parent().children();
            	for(var i=0; i < divs.length; i++){
            		var div = $($(divs[i]).children('input')[0]);
                	if(div.val()!='' && !Number.isInteger(parseInt(div.val()))){
                		otp.widgets.Dialogs.showOkDialog("Number of containers must be an integer", "Error");
                		$('#otp-spinner').hide();
                		return false;
                	}
                	if(div.val()!='' && parseInt(div.val()) < 0){
                		otp.widgets.Dialogs.showOkDialog("Number of containers must be greater than 0", "Error");
                		$('#otp-spinner').hide();
                		return false;
                	}
            	}
            	
            	divs = $('#otp-planner-optionsWidget-Group-Configuration-content input');
            	for(var i=0; i < divs.length; i++){
            		$(divs[i]).css('border-color', 'initial');
            	}
            	for(var i=0; i < divs.length; i++){
            		var div = $(divs[i]); 
            		if(isNaN(div.val()) && div.val()!='reset'){
                		div.css('border-color', 'red');
                		div.parent().after(this_.errMsg.$());
                		this_.errMsg.setText('* please enter a number');
                		this_.errMsg.setBinding(div);
                		$('#otp-spinner').hide();
                		return false;
                	}
            		else if(div.val() < 0 && div.val()!='reset'){
            			div.css('border-color', 'red');
                		div.parent().after(this_.errMsg.$());
                		this_.errMsg.setText('* the value must be greater than 0');
                		this_.errMsg.setBinding(div);
                		$('#otp-spinner').hide();
                		return false;
            		}
...
//        console.log(itin);
    ...
    highlightLeg : function(leg) { //PHUONG: highlightLeg when mouse on the leg on Itineraries panel
        if(!leg.legGeometry) return;
        var polyline = new L.Polyline(otp.util.Geo.decodePolyline(leg.legGeometry.points));
        polyline.setStyle({ color : "red", weight: 20, opacity: 0.3 });
    ...
    drawStartBubble : function(leg, highlight) {
        ...
        var modeIcon = this.icons.getModeBubble(quadrant, leg.startTime, leg.mode, true, highlight); //PHUONG: getModeBubble is defined in IconFactory.js
    ...
    clearTrip : function() {
        ...
        if(this.end2Marker) this.markerLayer.removeLayer(this.end2Marker);
        this.end2Name = this.end2LatLng = this.end2Marker = null;
